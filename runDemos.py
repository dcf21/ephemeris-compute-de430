...
"""
Generate a set of sample ephemerides. This is a useful thing to do immediately post-installation, as it triggers
EphemerisCompute to convert its ASCII input files into binary format, which can quickly be accessed for subsequent
queries.
"""
...
# Objects to generate sample ephemerides for. This should include a planet, asteroid and comet, to ensure that all
# ASCII input files are used to converted into binary format.
object_list: List[str] = ['jupiter', 'A1', '0002P']
...
def run_demos(selected_ephemeris: int = 440, all_ephemerides: bool = False) -> None:
    ...
    for de_number in ephemeris_list:
        for object_name in object_list:
            command: str = "{} -d {:d} -o {:s}".format(binary_path, de_number, object_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--all-ephemerides', dest='fetch_all', action='store_true',
                        help='Generate samples from all NASA JPL DE4xx ephemerides.')
